...
function PricingCard({image,title,desc,date,support,carddata,btnbio,spandata}) {
  return (
    <>
         <div className='bg-white rounded-lg shadow-md md:w-[45%] lg:w-[31%] p-6 w-[90%] relative overflow-hidden'>
   ...
   alt={title} />
    ...
    >{title}</h3>
    <p className='text-text-gray-color text-center mb-5'>{desc}</p>
    <span className='text-text-gray-color pb-6 block text-center'>{support}</span>
